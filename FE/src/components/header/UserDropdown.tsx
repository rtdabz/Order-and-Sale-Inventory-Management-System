...
export default function UserDropdown() {
  ...
  useEffect(() => {
    // Check if user is logged in as admin (has token)
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          <img src="/images/user/MKB.jpg" alt="User" />
        </span>

        <span className="block mr-1 font-medium text-theme-sm">{isAdmin ? "Admin" : "Customer"}</span>
        <svg
      ...
      >
        ...
        <button
          onClick={async (e) => {
            ...
            // Show a non-blocking toast and redirect to root sign-in route
            try {
